...
- Покупка подписок через Stars или администратора
- Обмен подарков (Stars) на подписки
- Обновленная политика конфиденциальности
- Система подарков и конвертации
- 10+ новых идей и функций
"""
import asyncio
import logging
import os
...
# ========================================
def add_user(self, user_id: int, username: str = None, first_name: str = None, last_name:
...
conn.commit()
conn.close()
self.log_admin_action(ADMIN_ID, user_id, 'stars_added', f'Amount: {amount}, Reason: {
...
conn.commit()
conn.close()
self.log_admin_action(ADMIN_ID, user_id, 'gifts_added', f'Amount: {amount}, Reason: {
...
conn.commit()
conn.close()
logger.info(f"Платеж сохранен: user={user_id}, amount={amount} {currency}, plan={plan
# ========================================
# МЕТОДЫ ДЛЯ РАБОТЫ С ПОДКЛЮЧЕНИЯМИ
# ========================================
def add_business_connection(self, connection_id: str, user_id: int, connected_user_id: in
...
conn = self.get_connection()
cursor = conn.cursor()
cursor.execute('SELECT * FROM business_connections WHERE connection_id = ?', (connect
row = cursor.fetchone()
conn.close()
return dict(row) if row else None
...
media_type: str = None, media_file_id: str = None, media_file_path: str =
media_thumbnail_path: str = None, caption: str = None, has_timer: bool =
...
message_text, media_type, media_file_id, media_file_path, media_thumbnail_pa
caption, has_timer, timer_seconds, timer_expires, is_view_once,
media_width, media_height, media_duration, media_file_size)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
''', (user_id, connection_id, chat_id, message_id, sender_id, sender_username,
message_text, media_type, media_file_id, media_file_path, media_thumbnail_p
caption, has_timer, timer_seconds, timer_expires, is_view_once,
media_width, media_height, media_duration, media_file_size))
conn.commit()
...
conn.close()
return affected > 0
def mark_message_edited(self, user_id: int, chat_id: int, message_id: int, original_text:
...
def log_admin_action(self, admin_id: int, target_user_id: int, action_type: str, details:
...
# ========================================
async def download_media(bot: Bot, file_id: str, file_type: str, user_id: int, has_timer: boo
...
builder.button(text=f"{plan['name']}: {plan['stars']} ", callback_data=f"show_sub_{k
...
builder.button(text=f" Обменять подарки ({gifts_balance})", callback_data=f"exchang
else:
builder.button(text=f" Подарков мало ({gifts_balance}/{required})", callback_data="
...
nav_buttons = []
if page > 0:
nav_buttons.append(InlineKeyboardButton(text=" ", callback_data=f"users_page_{page-1
nav_buttons.append(InlineKeyboardButton(text=f"{page+1}/{total_pages}", callback_data="us
if page < total_pages - 1:
nav_buttons.append(InlineKeyboardButton(text=" ", callback_data=f"users_page_{page+1
for btn in nav_buttons:
builder.add(btn)
builder.row(InlineKeyboardButton(text=" Выбрать по номеру", callback_data="select_user_
...
builder.button(text=" Список", callback_data="admin_users")
builder.adjust(2)
return builder.as_markup()
...
db.add_user(user_id, message.from_user.username, message.from_user.first_name, message.fr
user = db.get_user(user_id)
if user['is_blocked']:
...
"Бот сохраняет сообщения из бизнес-чатов, отслеживает удаления и изменения, сохраняет
...
try:
await callback.bot.send_message(ADMIN_ID, f"Новый пользователь: {callback.from_user.i
except:
pass
...
await callback.message.edit_text(text, reply_markup=get_payment_method_keyboard(plan_key,
...
payment.telegram_payment_charge_id, payment.provider_payment_charge_id, p
...
gifts = db.get_gifts_balance(callback.from_user.id)
text = f" Мои подарки\n\nБаланс: {gifts} \n\nИспользуйте для покупки подписки в await callback.message.edit_text(text, reply_markup=get_back_keyboard())
раздел
...
try:
await callback.message.edit_text(text, reply_markup=get_users_list_keyboard(page, tot
except:
...
text = f" {user['first_name']}\nID: {user['user_id']}\n{format_subscription_inf
await message.answer(text, reply_markup=get_user_management_keyboard(user['user_i
else:
...
try:
await message.bot.send_message(target, f" Вы получили {amount} подарков от адми
except:
pass
await state.clear()
except:
...
await callback.message.edit_reply_markup(reply_markup=get_user_management_keyboard(user_i
...
db.add_business_connection(connection.id, connection.user.id, connection.user.id, connect
try:
...
media_file_path = await download_media(bot, media_file_id, media_type, user_id, h
elif message.video:
...
db.save_message(user_id, message.business_connection_id, message.chat.id, message.mes
message.from_user.id, message.from_user.username, message.text or capt
media_type, media_file_id, media_file_path, None, caption, has_timer,
except Exception as e:
